...
sampler2D _VPSeeThroughScreenMask;
	...
	uniform float  _VPSeeThroughAlpha;
	...
	#define VOXELPLAY_OUTPUT_SEE_THROUGH_DATA(o, wpos) o.worldPos = wpos; o.scrPos = ComputeScreenPos(o.pos); o.scrPos.xy = o.scrPos.xy * 2.0 + _Time.xx;
	...
	void ApplySeeThrough(inout fixed4 color, float3 worldPos, float4 scrPos) {
		...
		color.a *= max(_VPSeeThroughAlpha, saturate(orthoDistSqr - radiusSqr));
	}
	...
	#define VOXELPLAY_OUTPUT_SEE_THROUGH_DATA(o, wpos)
