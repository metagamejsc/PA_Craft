...
fixed _VPGlobalSpecular;

fixed GetSpecular(float3 normal, float3 viewDir) {
	half3 refl = 2.0 * saturate(dot(_WorldSpaceLightPos0.xyz, normal)) * normal - _WorldSpaceLightPos0.xyz;
	fixed specular = dot(refl, viewDir);
	specular = specular * specular * sign(specular);
	return specular * _VPGlobalSpecular;
}

#if VOXELPLAY_PIXEL_LIGHTS && defined(USES_GLOBAL_SPECULAR)
	#define VOXELPLAY_APPLY_SPECULAR(color, norm, wpos) color.rgb *= 1.0 + GetSpecular(norm, normalize(_WorldSpaceCameraPos - wpos));
#else
	#define VOXELPLAY_APPLY_SPECULAR(color, norm, wpos)
#endif
