...
// Emission =====================================================================
    ...
    #define VOXELPLAY_COMPUTE_EMISSION_COLOR(color, i) fixed3 emissionColor = color.rgb * _EmissionMap.Sample(SAMPLER_NAME, i.uv.xy).rgb * _EmissionColor
	#define VOXELPLAY_APPLY_EMISSION(color) color.rgb += emissionColor;
#else
    #define VOXELPLAY_COMPUTE_EMISSION_COLOR(color, i)
	#define VOXELPLAY_APPLY_EMISSION(color)
#endif
...
v2f vert (appdata v) {
	...
	fixed4 color = v.color;
        ...
        // vertex colors are usually stored in gamma color space (ie. the Qubicle importer tool does this)
...
//	UNITY_SETUP_INSTANCE_ID(i);
	...
	#if defined(USE_TEXTURE) || defined(USE_TRIPLANAR)
		color *= i.color;
	#endif

    #if defined(USE_CUSTOM_BUMP_MAP)
       float4 nrm = _BumpMap.Sample(sampler_Linear_Repeat, i.uv.xy);
       VOXELPLAY_APPLY_CUSTOM_BUMPMAP(i, nrm);
    #endif
    VOXELPLAY_COMPUTE_EMISSION_COLOR(color, i);
	VOXELPLAY_APPLY_LIGHTING_AND_GI(color, i);
    VOXELPLAY_APPLY_EMISSION(color);
