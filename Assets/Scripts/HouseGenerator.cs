...
public class HouseGenerator : MonoBehaviour
{
    ...
    public static void GenerateLargeHouse(Dictionary<ChunkPos, TerrainChunk> chunks, int startX, int startY, int startZ)
    {
        int height = 4; // Chiều cao ngôi nhà (số tầng)
        int houseWidth = 13; // Rộng
        int houseDepth = 13; // Sâu

        // Lát nền bằng PlankBrich
        ...
        // Xây tường xung quanh (Stone) và cửa chính giữa mặt trước
        for (int y = startY + 1; y < startY + height + 1; y++)
        {
            for (int x = 0; x < houseWidth; x++)
            {
                for (int z = 0; z < houseDepth; z++)
                {
                    ...
                    {
                        // Cửa chính giữa mặt trước (z == 0)
                        if (y >= startY + 1 && y <= startY + 2 && z == 0 && x == houseWidth / 2)
                            continue;

                        // Cửa sổ tầng trên (Glass)
                        else if ((y == startY + 3 || y == startY + 2) &&
                                 (x == 0 || x == houseWidth - 1 || z == 0 || z == houseDepth - 1))
        ...
        // Đặt cột gỗ ở 4 góc
        for (int y = startY + 1; y < startY + height + 2; y++)
        {
        ...
        SetBlockGlobal(chunks, startX + 1, startY + 3, startZ + houseDepth - 1, BlockType.Glass, useRandom: false);
        SetBlockGlobal(chunks, startX + houseWidth - 2, startY + 3, startZ + houseDepth - 1, BlockType.Glass,
            useRandom: false);

        // Mái nhà hình chóp
        for (int layer = 0; layer < houseWidth / 2; layer++)
        {
            int currentY = startY + height + 1 + layer;
            for (int x = layer; x < houseWidth - layer; x++)
            {
                for (int z = layer; z < houseDepth - layer; z++)
                {
                    ...
                    {
                        SetBlockGlobal(chunks, startX + x, currentY, startZ + z, BlockType.Brick, useRandom: false);
                    }
                }
            }
        }
    ...
    public static void GenerateSlopedRoofHouse(Dictionary<ChunkPos, TerrainChunk> chunks, int startX, int startY,
        int startZ)
    {
        int houseWidth = 7;
        int houseDepth = 7;
        ...
        for (int layer = 0; layer <= height; layer++)
        {
            ...
            for (int dx = -halfWidth; dx <= halfWidth; dx++)
            {
                ...
                {
                    SetBlockGlobal(chunks, startX + xLeft, startY + height + layer, startZ, BlockType.Brick,
                        useRandom: false);
                    SetBlockGlobal(chunks, startX + xLeft, startY + height + layer, startZ + houseDepth - 1,
                        BlockType.Brick, useRandom: false);
                }
            }
            ...
            for (int dz = 1; dz < houseDepth - 1; dz++)
            {
                ...
                if (xLeft >= 0 && xLeft < houseWidth)
                    SetBlockGlobal(chunks, startX + xLeft, startY + height + layer, startZ + dz, BlockType.Brick,
                        useRandom: false);
                if (xRight >= 0 && xRight < houseWidth)
                    SetBlockGlobal(chunks, startX + xRight, startY + height + layer, startZ + dz, BlockType.Brick,
                        useRandom: false);
            }
        }
    }

    public static void GenerateHouseWithNewWindows(Dictionary<ChunkPos, TerrainChunk> chunks, int startX, int startY,
        int startZ)
    {
        int height = 4;
        int houseWidth = 7;
        ...
        SetBlockGlobal(chunks, startX + houseWidth / 2, startY + height + 1, startZ + houseDepth - 1, BlockType.Glass,
            useRandom: false);
        ...
        SetBlockGlobal(chunks, startX + houseWidth - 2, startY + 3, startZ + houseDepth - 2, BlockType.Glass,
            useRandom: false);
    ...
    public static void SetBlockGlobal(Dictionary<ChunkPos, TerrainChunk> chunks, int x, int y, int z, BlockType type,
        bool useRandom = true)
    {
