...
public class EnemyController : MonoBehaviour
{
    ...
    public float deathFallDuration = 0.45f;
    public float deathFallAngle = 95f;
    public float deathFallSinkDistance = 0.15f;
    ...
    public void Die()
    {
        ...
        Quaternion fallRotation = startRotation * Quaternion.Euler(deathFallAngle, 0f, 0f);
        Vector3 endPosition = startPosition - Vector3.up * Mathf.Max(0f, deathFallSinkDistance);

        deathTween = DOTween.Sequence()
            .Append(transform.DOMove(endPosition, Mathf.Max(0.05f, deathFallDuration)))
            .Join(transform.DORotateQuaternion(fallRotation, Mathf.Max(0.05f, deathFallDuration)))
            .SetEase(Ease.OutQuad)
            .SetLink(gameObject);
