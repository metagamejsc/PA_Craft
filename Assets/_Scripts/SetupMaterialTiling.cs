...
public class SetupMaterialTiling : MonoBehaviour
{
    ...
    private void ApplyMaterialPropertyBlock(Renderer targetRenderer)
    {
        ...
        Vector4 textureTransform = new Vector4(tiling.x, tiling.y, scrollOffset.x, scrollOffset.y);
        mpb.SetVector("_MainTex_ST", textureTransform);
        mpb.SetVector("_BaseMap_ST", textureTransform);
    ...
    private void SetupTextureWrapMode()
    {
        ...
        Texture mainTexture = sharedMaterial.HasProperty("_BaseMap")
            ? sharedMaterial.GetTexture("_BaseMap")
